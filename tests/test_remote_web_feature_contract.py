...
def test_remote_web_feature_contract_covers_real_headless_routes():
    ...
    assert any(source.endswith("app\\backend\\server\\artist_thumbnail_routes.py") or source.endswith("app/backend/server/artist_thumbnail_routes.py") for source in payload["route_sources"])
    assert any(source.endswith("app\\backend\\server\\character_viewer_routes.py") or source.endswith("app/backend/server/character_viewer_routes.py") for source in payload["route_sources"])
    ...
    assert any(source.endswith("app\\backend\\server\\event_preset_routes.py") or source.endswith("app/backend/server/event_preset_routes.py") for source in payload["route_sources"])
