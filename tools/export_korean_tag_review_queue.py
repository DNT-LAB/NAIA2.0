...
def main():
    ...
    policy = load_policy()
    excluded = [{'tag': tag, **reason} for tag in sorted(missing)
                if (reason := exclusion(tag, records[tag], policy))]
    pending = missing - {r['tag'] for r in excluded}
    ...
    with (args.out / "pending.jsonl").open("w", encoding="utf-8") as output:
        ordered = sorted(pending, key=lambda t: (-max(int(r.get("freq") or 0) for r in records[t]), t))
    ...
    manifest = {"schema_version": 1, "general_tags": len(eligible), "missing_korean_tags": len(missing),
        "pending_translation_tags": len(pending), "excluded_translation_tags": len(excluded),
        ...
        "promotion": "Review definitions into korean_alias_reviewed.json or bounded korean_alias_rules.json, rebuild supplement, inspect collisions and prior-alias retention, then run production-route regression. Drafts and this queue are never runtime aliases.",
    ...
    (args.out/'excluded.json').write_text(json.dumps(excluded, ensure_ascii=False, indent=2), encoding='utf-8')
