...
7. **시드 재생성본이 원본보다 나쁠 수 있다.** 2026-07-26 의상 재생성 30장 중 3장이
   `1girl, solo` 인데도 **2인으로 렌더**됐다. 재생성 폴더를 통째로 `build_*` 에 넘기면
   멀쩡한 원본이 덮인다(`갱신` 카운트로 표시된다). **눈으로 보고 선별 병합한다.**

8. **시드로 고칠 수 있는 것과 없는 것을 먼저 가른다.** 재생성 전에 원인을 나눈다 —
   (a) 부재 태그(`no bra` 류)는 렌더할 대상이 없다, (b) `front view` 고정이면 등 쪽
   태그(`back cutout`)는 원리적으로 안 보인다, (c) 대상이 프레임에서 차지하는 화소가
   너무 작으면(귀 ≈ 15px @192px 크롭) 시드를 아무리 바꿔도 같다. 이 셋은 **프레이밍
   사안**이라 재생성에 요청을 쓰면 전부 낭비다. 판별이 애매하면 **파일럿 3장**으로
   프레이밍을 바꿔 찍어보는 것이 가장 싸다.

9. **캐시 마커는 3곳이다.** `interactivePanel.mjs`(ax) / `app.js`(ia) /
   **`index.html` 의 `app.js?v=`**. 마지막 것을 빼먹으면 app.js 가 캐시돼 나머지 두 개를
   올린 의미가 사라진다.
