// I.Sequence (Inpaint Sequence) 패널 — Sequence 패널의 **복제본**(사용자 지정 2026-08-25).
//
// ⚠️ 검색·다운로드는 `/api/sequence-preset/*` 을 **그대로** 쓴다 — 같은 이벤트 데이터셋이라
//    띄울 이유가 없다. 따로 가지는 것은 생성 3종(`/api/inpaint-sequence/*`)뿐이다.
// ⚠️ 이번 단계는 복제까지다 — 프레임은 아직 원본과 똑같이 독립 t2i 로 나간다. 목표인
//    캔버스 연쇄(직전 이미지를 붙이고 빈 절반만 inpaint)는 다음 단계에서 갈아끼운다.
//
// 원 주석: 검증된 실제 이벤트 그룹을 태그 검색으로 서빙 (Dev0714 EventSearcher 모델).
// 검색바를 영속화(input autocomplete/포커스 보존) 하고 결과 영역만 갱신한다. 수위·컷 칩은
// 검색 버튼 없이 즉시 반영. 포함/제외 입력에 tagAssist(태그 자동완성) 바인딩.
// 좌측 #tabIsequence 상주. 생성은 표준 PE 경유 — Storyteller 비커플링.
...
export function createInpaintSequencePanel({panel, escHtml, showToast, bindTagAssist, getApiMode}) {
  const state = {
  ...
  async function ensureReady() {
    ...
    try {
      ...
      state.available = false; renderUnavailable();
      showToast(`I.Sequence 로드 실패: ${error.message}`, 'error');
    }
  }
  ...
  async function generateGroup(groupId) {
    ...
    try {
      const out = await postJson('/api/inpaint-sequence/generate',
  ...
  // 메인 Random(Alt+Enter) — I.Sequence 컨텍스트. 현재 매칭 전체에서 랜덤 그룹 1개를 연속 생성.
  // Auto Gen ON 이면 백엔드 러너가 라운드 완료마다 다음 랜덤 그룹으로 이어간다.
  async function runRandomGenerate() {
    if (state.busy) return true;
    state.busy = true;
    try {
      const out = await postJson('/api/inpaint-sequence/random-generate', searchPayload());
      const frames = out.frames || [];
  ...
  return {
    ...
    // 메인 하단 버튼 라우팅용(app.js). I.Sequence 탭 활성일 때 Ctrl/Alt+Enter 가 위임된다.
    hasOpenGroup() {
