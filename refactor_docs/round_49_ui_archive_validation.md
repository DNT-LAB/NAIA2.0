...
Generated: 2026-05-19T16:58:34+09:00
...
```powershell
python tools/measure_web_session_startup.py --entrypoint headless --port 7314 --cdp-port 9414 --include-generate
```
...
| `remote_web_first_paint` | 2.156 |
| `random_click_to_prompt_update` | 6.359 |
...
| `after_listen` | 119.27 |
| `after_status` | 119.68 |
| `after_first_paint` | 121.41 |
| `after_action_ready` | 125.69 |
| `after_random` | 1380.42 |
| `after_generate_dispatch` | 1380.43 |
| `after_shutdown` | None |
...
| Log | Path |
| --- | --- |
| `stdout` | C:\VNR\NAIA2.0\logs\round30_web_session_7314_20260519_165834.out.log |
| `stderr` | C:\VNR\NAIA2.0\logs\round30_web_session_7314_20260519_165834.err.log |
| `import_audit` | C:\VNR\NAIA2.0\logs\round30_import_audit_7314_20260519_165834.jsonl |
...
## Round 49G Static Checks

- `python -m compileall -q legacy_desktop`
- `python -m pytest tests\test_theme_cache.py -q` -> 1 passed
...
- `rg "from ui\.|import ui\." --glob "*.py" --glob "!venv/**"` -> no matches
- `git ls-files ui` -> only `ui/remote_web/**` tracked assets

## Round 49G Result

Tracked PyQt UI wrappers are now archived under `legacy_desktop/ui/`. The supported root `ui/` tree contains only Remote Web static assets.
