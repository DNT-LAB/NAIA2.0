...
SLOTS = [
    ...
    ("눈·얼굴", "\\u{1F441}", "characteristic", [
        ...
        # (`눈·입 형태`/`face_shape` 는 표정으로 옮겼다 — 아래 표정 축 주석 참조.)
    ...
    ("표정", "\\u{1F60A}", "expression", [
        ...
        ("thumb", "성격·유형", "persona"),
        # 눈·얼굴에 있었다. "형태"라는 이름이 생김새처럼 들리지만 21개 전량이
        # `open mouth`·`closed eyes`·`puffy cheeks` 처럼 **지금 짓고 있는** 것이다.
        # 한때 "형태는 감정이 아니라 구조다"로 눈·얼굴에 뒀는데, 진짜 구조형
        # (`tsurime`·`tareme`·`jitome`·`sanpaku`·`thick eyebrows`)은 `face` 축에 따로 있다.
