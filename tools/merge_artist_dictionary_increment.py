...
⚠️ 중앙값 1.00 이 꼬리까지 보장하지는 않는다(실측): 비율 <0.1 이 3,973종,
   <0.5 가 12,995종, >2 가 4,567종이다. 코퍼스는 Danbooru 전체가 아니라 표본이라
   `otohime (youngest princess)`(인덱스 925 / 코퍼스 1) 처럼 코퍼스에 거의 없는
   옛 아티스트가 있고, 반대로 `inoino`(인덱스 34 / 코퍼스 4,353) 처럼 인덱스가
   낡은 경우도 있다. 그래서 신규분과 기존분의 랭킹이 완전히 정합하지는 않는다.
   **전체를 코퍼스로 다시 계산하는 것은 해법이 아니다** - 코퍼스에 1건뿐인
   3,973종의 순위를 통째로 무너뜨린다. 지금은 "없던 항목이 근사값으로라도
   생기는 것" 이 "아예 없는 것" 보다 낫다는 판단이다.
...
def main() -> int:
    ...
    if not new:
        print("  추가할 항목이 없습니다 - 아무것도 쓰지 않습니다")
        return 0

    text = DICT_PATH.read_bytes().decode("utf-8")
    # 마커가 있어도 멈추지 않는다. 중복은 위에서 임포트한 `artist_dict` 기준으로 이미
    # 걸렀고(이전 증분분도 그 안에 들어 있다) 마커는 사람이 읽기 위한 표시일 뿐이다.
    # 예전에는 여기서 exit 1 을 내서, 한 번 돌리고 나면 다음 증분이 영영 막혔다.
    if MARKER in text:
        print(f"  (이미 증분 블록이 있습니다 - 블록을 하나 더 붙입니다)")
    idx = text.rfind("}")
    if idx == -1:
