...
- COMFYUI 모드에서는 COMFYUI 전용 도구 카테고리를 노출한다. `커스텀 워크플로우`는 PNG/WEBP를 `/api/comfyui/workflow/upload`로 업로드하고, `기본 워크플로우 전환`은 커스텀 사용 중에만 활성화한다. `ComfyUI 웹 열기`는 `/api/comfyui/web` 리다이렉트를 사용한다.
