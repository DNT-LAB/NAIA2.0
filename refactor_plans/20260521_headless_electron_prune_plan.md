...
- Required validation for this slice: install-manager route test, runtime install manager tests, backend package shim tests, remote web feature contract test/checker, prompt highlight route smoke, Prompt Engineering preset-thumbnail upload/get/generate smoke, style thumbnail route smoke, Danbooru route smoke, Event/Preset bootstrap/select/preview/generate/thumbnail smoke, clothes/expression preset route smoke, Character Viewer route smoke, Artist Thumbnail route smoke, focused Event/Preset websocket tests, and syntax checks of moved route sources.
