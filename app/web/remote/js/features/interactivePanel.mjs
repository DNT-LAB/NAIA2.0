...
} from './interactiveAxes.mjs?v=20260913-gap3385';
...
export function createInteractivePanel({
  ...
  // 그림이 떠 있을 때 **남겨 둘 폭**.
  //
  // 처음엔 360 이었다 - "그림이 편하게 보이는 크기". 그런데 그 값이 사용자 화면에서
  // 열을 3 에 묶는 유일한 원인이었고, 사용자 판정은 이랬다(2026-08-19):
  // **"구석에 들어가는건 맞는데, 그림처럼 작게 처박혀야하거든요."**
  // 즉 그림이 **작아지는 것**은 괜찮고, **가려지는 것**만 안 된다. 두 불만("이미지를
  // 가린다" / "열이 안 는다")은 서로 다른 요구였는데 한 상수가 둘을 겸하고 있었다.
  // 그래서 "편히 볼 크기"가 아니라 **구석에 처박힐 자리**로 낮춘다.
  const PANEL_IMG_TUCK = 200;
  ...
  // 열 상한. **5 는 사용자가 정한 값이다**(2026-08-19: "Line당 5개가 상한으로
  // (정신사나움)"). 자리가 더 남아도 5 에서 멈추고 나머지는 그림에 돌려준다 -
  // 한 줄이 길어지면 눈이 가로로 멀리 이동해 훑기가 오히려 느려진다.
  //
  // 이 값이 실질 폭 상한을 정한다: 134 + 5x108 + 4x6 = 698. 그래서 `PANEL_W_MAX`
  // (1040)는 이제 스냅 경로에서는 안 걸린다 - 상한을 다시 올릴 때를 위한 천장으로만
  // 남겨 둔다(둘 다 올려야 실제로 넓어진다).
  ...
  function positionPopup() {
    ...
    if (wide && viewerBox && viewerBox.width > 0) {
      ...
      // ⚠️ **사전과 그림은 같은 가로 자리를 쓴다 - 더하면 안 된다.**
      //
      // 사전은 팝업 오른쪽에 서고, 그림도 팝업 오른쪽 **전체**를 받는다. 둘은 가로로
      // 겹치고(실측: 사전 1285~1625 · 그림 1462~1975), 겹치는 만큼 그림이 아래로
      // 밀린다(`--ia-shift-top`). 그런데 여기서는 나란히 놓인 것처럼 **합쳐서** 뺐다.
      // 그 314 + 200 = 514 중 실제로 필요한 314 를 뺀 나머지가, 사용자가 세 번
      // 표시한 오른쪽 위 빈 칸이다.
      //
      // 겹치는 둘에 필요한 자리는 합이 아니라 **더 넓은 쪽**이다.
      const need = Math.max(ASIDE_W + 10, viewerHasShot() ? PANEL_IMG_TUCK : 0);
      const spare = right - left - 12 - need;
