...
- Resolution manager and ComfyUI workflow routes moved from `core/web_session_app.py` to `app/backend/server/params_workflow_routes.py`, keeping per-mode resolution storage, Comfy workflow state, and server-owned option broadcast behavior unchanged.
...
- `release_assets/manifests/remote_web_feature_contract.json` now supports multiple route source files and lists the install-manager, state, params/workflow, prompt-tools, style-thumbnail, Danbooru, Event Preset, Character Viewer, and Artist Thumbnail route owners.
- Required validation for this slice: install-manager route test, runtime install manager tests, backend package shim tests, remote web feature contract test/checker, prompt highlight route smoke, Prompt Engineering preset-thumbnail upload/get/generate smoke, resolution route smoke, ComfyUI workflow state/default/upload/web smoke, style thumbnail route smoke, Danbooru route smoke, Event/Preset bootstrap/select/preview/generate/thumbnail smoke, clothes/expression preset route smoke, Character Viewer route smoke, Artist Thumbnail route smoke, focused Event/Preset websocket tests, and syntax checks of moved route sources.
