...
- Resolution manager routes moved from `core/web_session_app.py` to `app/backend/server/params_workflow_routes.py`, keeping per-mode resolution storage and server-owned option broadcast behavior unchanged.
...
- Required validation for this slice: install-manager route test, runtime install manager tests, backend package shim tests, remote web feature contract test/checker, prompt highlight route smoke, Prompt Engineering preset-thumbnail upload/get/generate smoke, resolution route smoke, style thumbnail route smoke, Danbooru route smoke, Event/Preset bootstrap/select/preview/generate/thumbnail smoke, clothes/expression preset route smoke, Character Viewer route smoke, Artist Thumbnail route smoke, focused Event/Preset websocket tests, and syntax checks of moved route sources.
