...
class HeadlessPromptEngineeringService:
    ...
    def _nai_v5_recommended_module_settings() -> dict[str, Any]:
        ...
        return {
            ...
            # recommend_6 이 함께 담고 있는 보조 설정. **프리셋 파일을 그것과
            # 같게 만들기 위해** 넣는다.
            #
            # ⚠️ 처음에 "이것들도 같이 가야 추천이 재현된다" 고 적었는데 **틀렸다**
            #    (Codex CONCERN). e621/Danbooru/Ollama 부스트는 프리셋이 아니라
            #    **전역 파일**(`e621_boost_user.json` 등)에서 읽는다 - 프리셋 적용은
            #    그 파일을 쓰지 않으므로 여기 값은 저장만 되고 안 먹는다.
            #    사용자의 recommend_6 도 똑같이 저장만 된다 - 그래서 '동기화' 라는
            #    목적에는 맞다. 실제로 먹게 하려면 전역 파일까지 써야 하는데,
            #    그것은 사용자가 손수 맞춰 둔 값을 추천이 덮는다는 뜻이라 별건이다.
            "e621_settings": {
                "weight": 1.05,
                "hidden_tags": [],
                "mode": "confused",
