...
def check_bank_blob(blob: bytes) -> dict:
    ...
    ─────────────────────────────────────────────────────────────────────────
    ## 이 문을 고치려는 사람에게 (2026-08-17~18, 리뷰 7라운드의 결론)

    이 게이트는 **여섯 라운드 연속 뚫렸다.** 매번 같은 모양이었다: 필드 목록을
    손으로 늘리는 방식으로 고쳤고, 다음 라운드에 "그 필드도 런타임이 읽더라" 가
    나왔다(`tags` -> 껍데기 -> `coverage`/`p` -> `support` -> 원소 타입).
    그래서 계약을 이렇게 정했다. **셋을 같이 지켜라.**

      1. 전수 필드 검사(`_entry_wellformed`) - 목록의 **모든** 항목을 본다.
         "하나라도 쓸 수 있으면 통과" 는 안 된다 - 런타임은 목록을 전부 소비한다.
      2. **소비자 전수 실행**(`_smoke_lookup`) - 모든 앵커에 `lookup` 을 실제로
         돌린다. 1번 목록이 낡아도 이것이 잡는다. 표본으로 줄이지 마라 - "정의상
         전부 덮는다" 는 말은 전수일 때만 참이다(앵커당 9us, 63,468개에 1.1초).
      3. 계약은 **"불량 입력 -> ValueError"** 다. 검증기가 다른 예외로 탈출하면
         `verify_all` 이 목록 대신 예외를 던지고 빌더의 처리도 우회한다.

    범위(`[0,1]`·`support>=0`)와 상한(10**15)은 실측 근거가 있다 - 좁힐 때는
    `TestGate6Findings.test_real_ranges_stay_inside_the_gate` 를 함께 보라.
    회귀는 `tests/test_tag_combo.py` 의 `TestGate2~6Findings` 다.
    ─────────────────────────────────────────────────────────────────────────
    """
