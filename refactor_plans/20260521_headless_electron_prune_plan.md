...
- `release_assets/manifests/remote_web_feature_contract.json` now supports multiple route source files and lists the install-manager, state, prompt-tools, style-thumbnail, Danbooru, Event Preset, Character Viewer, and Artist Thumbnail route owners.
- Required validation for this slice: install-manager route test, runtime install manager tests, backend package shim tests, remote web feature contract test/checker, prompt highlight route smoke, Prompt Engineering preset-thumbnail upload/get/generate smoke, style thumbnail route smoke, Danbooru route smoke, Event Preset status/download smoke, clothes/expression preset route smoke, Character Viewer route smoke, Artist Thumbnail route smoke, and syntax checks of moved route sources.
