...
- Required validation for this slice: web-shell static route smoke, install-manager route test, runtime install manager tests, backend package shim tests, remote web feature contract test/checker, prompt highlight route smoke, tag lookup HTTP/WebSocket smoke, Result/History/Viewer route smoke, Img2Img/Inpaint service smoke, Character Reference service smoke, Vibe Transfer service smoke, Automation service smoke, WEBUI Hiresfix Assist service smoke, Event Stream service smoke, Prompt Engineering service smoke, Conditional Prompt service smoke, Prompt Engineering preset-thumbnail upload/get/generate smoke, resolution route smoke, search parquet upload route smoke, ComfyUI workflow state/default/upload/web smoke, style thumbnail route smoke, Danbooru route smoke, Event/Preset bootstrap/select/preview/generate/thumbnail smoke, clothes/expression preset route smoke, Character Viewer route smoke, Artist Thumbnail route smoke, focused Event/Preset websocket tests, and syntax checks of moved route sources.
