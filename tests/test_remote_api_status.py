...
def test_artist_thumb_anima_mode_reports_update_for_old_local_file(tmp_path, monkeypatch):
    ...
    assert anima["expected_size"] == 2656390724
    with pytest.raises(RuntimeError, match="needs update"):
        bridge._load_artist_thumb_data("ANIMA-22000")


def test_artist_thumb_anima_44000_mode_reports_metadata_and_missing_update(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    bridge = RemoteBridge(_AppContext())
    bridge._artist_thumb_artist_weights = lambda: {"a": 10}
    ...
    bucket2 = modes["ANIMA-44000"]
    assert bucket2["label"] == "ANIMA-44000"
    assert bucket2["available"] is False
    assert bucket2["needs_update"] is True
    assert bucket2["size"] == 0
    assert bucket2["expected_size"] == 2604574500
    assert bucket2["sha256"] == "3B581E8A5C596B4E2AE001C8842B486BF4D7BC36485D23B0861B0200C41017E2"


def test_artist_thumb_anima_44000_mode_uses_configured_local_file(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    Path("data").mkdir()
    ...
    bridge = RemoteBridge(_AppContext())
    mode_info = dict(bridge.ARTIST_THUMB_MODES["ANIMA-44000"])
    ...
    state = bridge._build_artist_thumb_state()
    bucket2 = next(mode for mode in state["modes"] if mode["key"] == "ANIMA-44000")

    assert bucket2["available"] is True
    assert bucket2["needs_update"] is False
    assert bridge._load_artist_thumb_data("ANIMA-44000") == {"a": ["thumb_a"]}


def test_artist_thumb_anima_44000_mode_reports_update_for_old_local_file(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    Path("data").mkdir()
...
def test_artist_thumb_download_validation_checks_sha256(tmp_path, monkeypatch):
    ...
    assert bridge._validate_artist_thumb_download_file("ANIMA-22000", target) == len(payload)
...
def test_artist_thumb_anima_44000_download_validation_checks_sha256(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    Path("data").mkdir()
    ...
    assert bridge._validate_artist_thumb_download_file("ANIMA-44000", target) == len(payload)
