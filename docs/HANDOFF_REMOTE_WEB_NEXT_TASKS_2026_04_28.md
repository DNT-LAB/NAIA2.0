...
- Result context menu follow-up:
  ...
  - Current-result reroll now reuses the desktop `ImageWindow._reroll_current_prompt()` path through a Qt queued bridge signal; saved images can reroll when their file path still maps to an in-memory desktop `HistoryItem`.
  - `이미지 저장` is enabled for both current results and saved result images as an explicit PNG save-as/export action.
